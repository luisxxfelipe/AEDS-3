...
bool ligaVertices(VERTICE v1, VERTICE v2)
{
    ...
    { // Busca a primeira posição 'vazia'(NULL) dos vizinhos
        aux++;
    }
...
int buscaEmProfundidade(VERTICE inicio, VERTICE destino, int visitados)
{
    ...
    { // Enquanto existe vizinhos a serem visitados
        if (inicio->vizinhos[aux]->visitado == false)
...
int buscaEmLargura(VERTICE inicio, VERTICE destino)
{
    ...
    { // Como a lista não é dinâmica, ela precisa ser 'limpa' primeiro
        FILA[i] = NULL;
    }
